...
Bridges the Wolfronix Engine to **PostgreSQL** (self-hosted, AWS RDS, Neon, Railway, etc.) for encrypted file and key storage.
...
```bash
docker build -t wolfronix-postgresql-connector .
docker run -d --env-file .env -p 8080:8080 wolfronix-postgresql-connector
```
...
- `wolfronix_file_data` — Encrypted file binary data (BYTEA)
