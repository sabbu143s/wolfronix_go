...
Bridges the Wolfronix Engine to **MySQL / MariaDB** for encrypted file and key storage.
...
```bash
docker build -t wolfronix-mysql-connector .
docker run -d --env-file .env -p 8080:8080 wolfronix-mysql-connector
```
...
- `wolfronix_files` — Encrypted file metadata (InnoDB)
- `wolfronix_file_data` — Encrypted file binary data (LONGBLOB)
